...
### This example is set to use the CNTL configuration
### User must make changes for BCRC, BDRC, etc
set job_name       = 1PCTCO2BGC_RUBISCO_CNPECACNT_20TR
set compset        = 20TR_CAM5%CMIP6_CLM45%CNPECACNTBC_MPASCICE%SPUNUP_MPASO%SPUNUP_MOSART_SGLC_SWAV_BGC%BDRC
set resolution     = ne30_oECv3
set machine        = compy
set walltime       = 24:00:00
set mail_user      = false
set email          = qzhu@lbl.gov
setenv project       rgma
set ymd            = `date +"%Y%m%d"`

### SOURCE CODE OPTIONS
set fetch_code      = true
#lastest version of maint-1.1
set e3sm_tag       = 5d73daaa50a7892671f8bef0d679356ceaa971a0
set tag_name       = ${ymd}_${job_name}_${machine}

### CUSTOM CASE_NAME
set case_name = ${ymd}.${job_name}.${resolution}.${machine}
...
set restart_files_dir = /compyfs/inputdata/e3sm_init/20181209.SPIN_CNPECACNT_1850.ne30_oECv3.edison/run/

### DIRECTORIES
set code_root_dir               = $home
set e3sm_simulations_dir        = /compyfs/$user/E3SM_simulations/1pctco2_rubisco/
...
if ( `lowercase $fetch_code` == true ) then
  ...
  git clone git@github.com:E3SM-Project/ACME-Hooks.git $code_root_dir/$tag_name/.git/hooks         # checkout with write permission.
...
else if ( `lowercase $processor_config` == 'custom_cbgc_1pct_compyshort' ) then

  e3sm_print 'using custom layout for compy because $processor_config = '$processor_config

  ${xmlchange_exe} MAX_TASKS_PER_NODE="40"

  ${xmlchange_exe} NTASKS_ATM="800"
  ${xmlchange_exe} ROOTPE_ATM="0"

  ${xmlchange_exe} NTASKS_LND="180"
  ${xmlchange_exe} ROOTPE_LND="619"

  ${xmlchange_exe} NTASKS_ICE="720"
  ${xmlchange_exe} ROOTPE_ICE="0"

  ${xmlchange_exe} NTASKS_OCN="800"
  ${xmlchange_exe} ROOTPE_OCN="799"

  ${xmlchange_exe} NTASKS_CPL="400"
  ${xmlchange_exe} ROOTPE_CPL="0"

  ${xmlchange_exe} NTASKS_GLC="40"
  ${xmlchange_exe} ROOTPE_GLC="719"

  ${xmlchange_exe} NTASKS_ROF="40"
  ${xmlchange_exe} ROOTPE_ROF="719"

  ${xmlchange_exe} NTASKS_WAV="40"
  ${xmlchange_exe} ROOTPE_WAV="719"
...
else if ( `lowercase $processor_config` == 'custom_cbgc_1pct_compyregular' ) then

  e3sm_print 'using custom layout for compy because $processor_config = '$processor_config

  ${xmlchange_exe} MAX_TASKS_PER_NODE="40"

  ${xmlchange_exe} NTASKS_ATM="2700"
  ${xmlchange_exe} ROOTPE_ATM="0"

  ${xmlchange_exe} NTASKS_LND="320"
  ${xmlchange_exe} ROOTPE_LND="2400"

  ${xmlchange_exe} NTASKS_ICE="2400"
  ${xmlchange_exe} ROOTPE_ICE="0"

  ${xmlchange_exe} NTASKS_OCN="800"
  ${xmlchange_exe} ROOTPE_OCN="2720"

  ${xmlchange_exe} NTASKS_CPL="960"
  ${xmlchange_exe} ROOTPE_CPL="0"

  ${xmlchange_exe} NTASKS_GLC="120"
  ${xmlchange_exe} ROOTPE_GLC="2400"

  ${xmlchange_exe} NTASKS_ROF="120"
  ${xmlchange_exe} ROOTPE_ROF="2400"

  ${xmlchange_exe} NTASKS_WAV="120"
  ${xmlchange_exe} ROOTPE_WAV="240"
...
else if ( `lowercase $processor_config` == 'custom_cbgc_edison' ) then

  # e3sm_print 'using custom layout for edison because $processor_config = '$processor_config

   ${xmlchange_exe} MAX_TASKS_PER_NODE="24"

   ${xmlchange_exe} NTASKS_ATM="5400"
   ${xmlchange_exe} ROOTPE_ATM="0"

   ${xmlchange_exe} NTASKS_LND="1000"
   ${xmlchange_exe} ROOTPE_LND="4200"

   ${xmlchange_exe} NTASKS_ICE="4200"
   ${xmlchange_exe} ROOTPE_ICE="0"

   ${xmlchange_exe} NTASKS_OCN="720"
   ${xmlchange_exe} ROOTPE_OCN="5400"

   ${xmlchange_exe} NTASKS_CPL="4800"
   ${xmlchange_exe} ROOTPE_CPL="0"

   ${xmlchange_exe} NTASKS_GLC="312"
   ${xmlchange_exe} ROOTPE_GLC="4200"

   ${xmlchange_exe} NTASKS_ROF="312"
   ${xmlchange_exe} ROOTPE_ROF="4200"

   ${xmlchange_exe} NTASKS_WAV="312"
   ${xmlchange_exe} ROOTPE_WAV="0"
...
# Use bilinear interpolation for CO2 emission files.  Note that this
# regridding is not mass-conserving and so is not optimal for CO2
# cycle simulations. SMB
$xmlchange_exe --id CAM_CONFIG_OPTS --append --val='-cppdefs -DCO2_BILIN_REGRID'
$xmlchange_exe --id CCSM_BGC --val='CO2C'
#===========================
...
if ( $model_start_type == 'hybrid' ) then
  ...
  set restart_filedate = ${restart_coupler_filename:r:e:s/-00000//}

# CLM and MOSART restart from a different run for this hybrid case
  set clmRestartFile = 20181209.SPIN_CNPECACNT_1850.ne30_oECv3.edison.clm2.r.0510-01-01-00000.nc
  ln -sf ${restart_files_dir}/$clmRestartFile $case_run_dir
  set finidat_clm = $clmRestartFile

  set mosartRestartFile = 20181209.SPIN_CNPECACNT_1850.ne30_oECv3.edison.mosart.r.0510-01-01-00000.nc
  ln -sf ${restart_files_dir}/$mosartRestartFile $case_run_dir
  set finidat_mosart = $mosartRestartFile
endif
...
cat <<EOF >> user_nl_clm
 ...
 finidat = '$finidat_clm'
 stream_fldfilename_ndep = '/compyfs/inputdata/lnd/clm2/ndepdata/fndep_elm_cbgc_exp_simyr1849-2101_1.9x2.5_c190103.nc' 
 hist_nhtfrq = 0,
 hist_mfilt = 1,
 hist_dov2xy = .true., 
 hist_fincl1 = 'TOTPRODC','LEAFC_STORAGE','LEAFC_XFER','FROOTC_STORAGE','FROOTC_XFER','LIVESTEMC_STORAGE','LIVESTEMC_XFER',
               'DEADSTEMC_STORAGE','DEADSTEMC_XFER','LIVECROOTC_STORAGE','LIVECROOTC_XFER','DEADCROOTC_STORAGE','DEADCROOTC_XFER','GRESP_STORAGE','GRESP_XFER',
               'CPOOL_TO_LEAFC','CPOOL_TO_FROOTC','CPOOL_TO_LIVESTEMC','CPOOL_TO_DEADSTEMC','CPOOL_TO_LIVECROOTC','CPOOL_TO_DEADCROOTC',
               'LEAFC_XFER_TO_LEAFC','FROOTC_XFER_TO_FROOTC','LIVESTEMC_XFER_TO_LIVESTEMC','DEADSTEMC_XFER_TO_DEADSTEMC','LIVECROOTC_XFER_TO_LIVECROOTC','DEADCROOTC_XFER_TO_DEADCROOTC',
               'LEAF_MR','FROOT_MR','LIVESTEM_MR','LIVECROOT_MR',
               'CPOOL_LEAF_GR','CPOOL_LEAF_STORAGE_GR','TRANSFER_LEAF_GR','CPOOL_FROOT_GR','CPOOL_FROOT_STORAGE_GR','TRANSFER_FROOT_GR',
               'CPOOL_LIVESTEM_GR','CPOOL_LIVESTEM_STORAGE_GR','TRANSFER_LIVESTEM_GR','CPOOL_DEADSTEM_GR','CPOOL_DEADSTEM_STORAGE_GR','TRANSFER_DEADSTEM_GR',
               'CPOOL_LIVECROOT_GR','CPOOL_LIVECROOT_STORAGE_GR','TRANSFER_LIVECROOT_GR','CPOOL_DEADCROOT_GR','CPOOL_DEADCROOT_STORAGE_GR','TRANSFER_DEADCROOT_GR',
               'LITFALL','LEAFC_TO_LITTER','FROOTC_TO_LITTER','M_LEAFC_TO_LITTER','M_LEAFC_STORAGE_TO_LITTER',
               ...
               'M_DEADCROOTC_TO_LITTER','M_DEADCROOTC_STORAGE_TO_LITTER','M_DEADCROOTC_XFER_TO_LITTER',
               'M_LEAFC_TO_LITTER_FIRE','M_LEAFC_STORAGE_TO_LITTER_FIRE','M_LEAFC_XFER_TO_LITTER_FIRE','M_FROOTC_TO_LITTER_FIRE','M_FROOTC_STORAGE_TO_LITTER_FIRE',
               ...
               'M_DEADSTEMC_TO_LITTER_FIRE','M_DEADSTEMC_STORAGE_TO_LITTER_FIRE','M_DEADSTEMC_XFER_TO_LITTER_FIRE','M_LIVEROOTC_TO_LITTER_FIRE',
               'M_LIVEROOTC_XFER_TO_LITTER_FIRE','M_LIVECROOTC_STORAGE_TO_LITTER_FIRE','M_DEADROOTC_TO_LITTER_FIRE','M_DEADCROOTC_STORAGE_TO_LITTER_FIRE',
               'M_DEADROOTC_XFER_TO_LITTER_FIRE','M_GRESP_STORAGE_TO_LITTER','M_GRESP_XFER_TO_LITTER','M_GRESP_STORAGE_TO_LITTER_FIRE','M_GRESP_XFER_TO_LITTER_FIRE',
               'COL_FIRE_CLOSS','LITFIRE','SOMFIRE','VEGFIRE','M_CPOOL_TO_FIRE','DWT_CLOSS','PRODUCT_CLOSS',
               'DWT_PROD100C_GAIN','DWT_PROD10C_GAIN','PROD10C','PROD100C','VEGC_TO_CWDC','M_CPOOL_TO_LITTER','M_CPOOL_TO_LITTER_FIRE',

               'TOTPRODN','LEAFN_STORAGE','LEAFN_XFER','FROOTN_STORAGE','FROOTN_XFER','LIVESTEMN_STORAGE','LIVESTEMN_XFER',
               'DEADSTEMN_STORAGE','DEADSTEMN_XFER','LIVECROOTN_STORAGE','LIVECROOTN_XFER','DEADCROOTN_STORAGE','DEADCROOTN_XFER','NPOOL','RETRANSN',
               'NPOOL_TO_LEAFN','NPOOL_TO_FROOTN','NPOOL_TO_LIVESTEMN','NPOOL_TO_DEADSTEMN','NPOOL_TO_LIVECROOTN','NPOOL_TO_DEADCROOTN',
               'LEAFN_XFER_TO_LEAFN','FROOTN_XFER_TO_FROOTN','LIVESTEMN_XFER_TO_LIVESTEMN','DEADSTEMN_XFER_TO_DEADSTEMN','LIVECROOTN_XFER_TO_LIVECROOTN','DEADCROOTN_XFER_TO_DEADCROOTN',
               'GAP_NLOSS_LITTER','FIRE_NLOSS_LITTER','HRV_NLOSS_LITTER','SEN_NLOSS_LITTER','NFIX_TO_ECOSYSN',
               'PROD1N','PROD10N','PROD100N','PRODUCT_NLOSS','PROD1N_LOSS','PROD10N_LOSS','PROD100N_LOSS','VEGN_TO_CWDN','M_NPOOL_TO_LITTER','M_NPOOL_TO_LITTER_FIRE',

               'TOTPRODP','LEAFP_STORAGE','LEAFP_XFER','FROOTP_STORAGE','FROOTP_XFER','LIVESTEMP_STORAGE','LIVESTEMP_XFER',
               'DEADSTEMP_STORAGE','DEADSTEMP_XFER','LIVECROOTP_STORAGE','LIVECROOTP_XFER','DEADCROOTP_STORAGE','DEADCROOTP_XFER','PPOOL','RETRANSP',
               'PPOOL_TO_LEAFP','PPOOL_TO_FROOTP','PPOOL_TO_LIVESTEMP','PPOOL_TO_DEADSTEMP','PPOOL_TO_LIVECROOTP','PPOOL_TO_DEADCROOTP',
               'LEAFP_XFER_TO_LEAFP','FROOTP_XFER_TO_FROOTP','LIVESTEMP_XFER_TO_LIVESTEMP','DEADSTEMP_XFER_TO_DEADSTEMP','LIVECROOTP_XFER_TO_LIVECROOTP','DEADCROOTP_XFER_TO_DEADCROOTP',
               'GAP_PLOSS_LITTER','FIRE_PLOSS_LITTER','HRV_PLOSS_LITTER','SEN_PLOSS_LITTER','VEGP_TO_CWDP','M_PPOOL_TO_LITTER','M_PPOOL_TO_LITTER_FIRE',
...
if ( $machine =~ 'cori*' || $machine == edison ) then
    if ( `lowercase $debug_queue` == true ) then
      $xmlchange_exe --subgroup case.run BATCH_COMMAND_FLAGS="--job-name=${job_name} --output=batch_output/${case_name}.o%j --time ${walltime} -q debug --account ${project}"
    else
      $xmlchange_exe --subgroup case.run BATCH_COMMAND_FLAGS="--job-name=${job_name} --output=batch_output/${case_name}.o%j --time ${walltime} -q regular --account ${project}"
    endif
...
else if ( $machine == compy ) then
  if ( `lowercase $debug_queue` == true ) then
    $xmlchange_exe --subgroup case.run BATCH_COMMAND_FLAGS="--job-name=${job_name} --output=batch_output/${case_name}.o%j --time ${walltime} -p short --account ${project}"
  else
    $xmlchange_exe --subgroup case.run BATCH_COMMAND_FLAGS="--job-name=${job_name} --output=batch_output/${case_name}.o%j --time ${walltime} -p slurm --account ${project}"
  endif
    $xmlchange_exe --subgroup case.st_archive BATCH_COMMAND_FLAGS="--job-name=ST+${job_name} --output=batch_output/ST+${case_name}.o%j --account=${project}"
...
#-----------------------------------------------------------------------------------------------
  ...
  e3sm_print 'Linking stuff for hybrid run'
  ln -sf ${restart_files_dir}/${restart_case_name}.cam.r.${restart_filedate}-00000.nc $case_run_dir
  ln -sf ${restart_files_dir}/${restart_case_name}.cam.rs.${restart_filedate}-00000.nc $case_run_dir
# For 'hybrid' run, CAM needs an initial condition file.
  ln -sf ${restart_files_dir}/${restart_case_name}.cam.i.${restart_filedate}-00000.nc $case_run_dir
  ln -sf ${restart_files_dir}/${restart_case_name}.clm2.r.${restart_filedate}-00000.nc $case_run_dir
  ln -sf ${restart_files_dir}/${restart_case_name}.clm2.rh0.${restart_filedate}-00000.nc $case_run_dir
  ln -sf ${restart_files_dir}/${restart_case_name}.cpl.r.${restart_filedate}-00000.nc $case_run_dir
  ln -sf ${restart_files_dir}/${restart_case_name}.mosart.r.${restart_filedate}-00000.nc $case_run_dir
  ln -sf ${restart_files_dir}/${restart_case_name}.mosart.rh0.${restart_filedate}-00000.nc $case_run_dir

# Use modified versions of MPAS-O and MPAS-CICE files with xtime
# variable renamed to xtime.orig (S.M. Burrows, following
# recommendation of Jon Wolfe)

#  ln -sf ${restart_files_dir}/mpascice.rst.${restart_filedate}_00000.nc $case_run_dir
#  ln -sf ${restart_files_dir}/mpaso.rst.${restart_filedate}_00000.nc $case_run_dir
#  [ -e $case_run_dir/mpascice.rst.${restart_filedate}_00000.nc ] && rm $case_run_dir/mpascice.rst.${restart_filedate}_00000.nc
#  [ -e $case_run_dir/mpaso.rst.${restart_filedate}_00000.nc ] && rm $case_run_dir/mpaso.rst.${restart_filedate}_00000.nc
  ...
  ncrename -v xtime,xtime.orig $case_run_dir/mpascice.rst.${restart_filedate}_00000.nc
  ncrename -v xtime,xtime.orig $case_run_dir/mpaso.rst.${restart_filedate}_00000.nc
# Make a second link to the MPAS-O restart file that will be used for
# initializing grids, including after future restarts. (S.M. Burrows,
# following suggestion from C. Golaz)
#  ln -sf $case_run_dir/mpaso.rst.${restart_filedate}_00000.nc $case_run_dir/init.mpaso.rst.${restart_filedate}.nc
  cp -s $case_run_dir/mpaso.rst.${restart_filedate}_00000.nc $case_run_dir/init.mpaso.rst.${restart_filedate}.nc
# Copy restart pointer files to run directory
  ...
  $xmlchange_exe --id CONTINUE_RUN --val "FALSE"
  $xmlchange_exe --id CAM_NAMELIST_OPTS --val "co2_cycle_rad_passive=.true. co2vmr_rad= 284.317000e-6"
...
#sed -i 's%/compyfs/inputdata/ocn/mpas-o/oEC60to30v3/mpaso.rst.BCRC_CNPECACNT_1850_anvil02.0066-01-01.no-xtime.180418.nc%mpaso.rst.0105-01-01_00000.nc%g' streams.ocean
sed -i 's%/compyfs/inputdata/ocn/mpas-o/oEC60to30v3/oEC60to30v3_60layer.170905.nc%mpaso.rst.0510-01-01_00000.nc%g' streams.ocean
# Change mesh file name in MPAS-O stream.  This always has to a file
# that will still be around after restarts and short-term archiving.
# WARNING: The way this is currently implemented relies on an
# assumption that the mesh file is listed first in the stream file
# (before the 'input' initial condition file).  This assumption is
# fragile. - Susannah M. Burrows
mv streams.ocean streams.ocean.tmp
awk 'NR==1,/mpaso.rst.0510-01-01_00000.nc/{sub(/mpaso.rst.0510-01-01_00000.nc/, "init.mpaso.rst.0510-01-01.nc")} 1' streams.ocean.tmp > streams.ocean
rm streams.ocean.tmp
# copy to SourceMods
cp streams.ocean ../case_scripts/SourceMods/src.mpaso/

# change sea ice init condition (only the first occurence, which is
# the 'input' file.  Leave the landIceMasks file at default. - SMB).
mv streams.seaice streams.seaice.tmp
awk 'NR==1,/\/compyfs\/inputdata\/ice\/mpas-cice\/oEC60to30v3\/seaice.EC60to30v3.161222.nc/{sub(/\/compyfs\/inputdata\/ice\/mpas-cice\/oEC60to30v3\/seaice.EC60to30v3.161222.nc/, "mpascice.rst.0510-01-01_00000.nc")} 1' streams.seaice.tmp > streams.seaice.tmp2
awk 'NR==1,/\/compyfs\/inputdata\/ice\/mpas-cice\/oEC60to30v3\/seaice.EC60to30v3.161222.nc/{sub(/\/compyfs\/inputdata\/ice\/mpas-cice\/oEC60to30v3\/seaice.EC60to30v3.161222.nc/, "mpascice.rst.0510-01-01_00000.nc")} 1' streams.seaice.tmp2 > streams.seaice
rm streams.seaice.tmp
rm streams.seaice.tmp2
# copy to SourceMods
cp streams.seaice ../case_scripts/SourceMods/src.mpascice/
popd
...
if ( `lowercase $submit_run` == 'true' ) then
  ...
  if ( `lowercase $mail_user` == 'true' ) then
    ${case_submit_exe}  --mail-user ${email}  -M all
  else
    ${case_submit_exe}
  endif
else
